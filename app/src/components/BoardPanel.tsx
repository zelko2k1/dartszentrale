...
export function BoardPanel() {
  ...
  const boardBadge = <span style={{ fontSize: 11, fontWeight: 800, color: 'var(--accent)', background: 'color-mix(in srgb, var(--accent) 14%, transparent)', border: '1px solid color-mix(in srgb, var(--accent) 40%, transparent)', padding: '3px 9px', borderRadius: 'var(--radius-xs)' }}>{boardName}</span>;

  if (!assignment || assignment.games.length === 0) {
    return wrap(
      <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius-lg)', padding: '14px 18px', display: 'flex', alignItems: 'center', gap: 10, color: 'var(--text-4)', fontSize: 13 }}>
        {boardBadge}
  ...
  if (!visible) {
    return wrap(
      <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius-lg)', padding: '14px 18px', display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap', color: 'var(--text-4)', fontSize: 13 }}>
        {boardBadge}
        ...
        <button onClick={() => s.showBoardNow()} style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 7, background: 'var(--accent)', border: 'none', color: 'var(--accent-fg)', padding: '7px 13px', borderRadius: 'var(--radius-sm)', fontSize: 13, fontWeight: 800, cursor: 'pointer', fontFamily: 'inherit' }}>
  ...
  return wrap(
    <div style={{ background: 'linear-gradient(135deg, color-mix(in srgb, var(--cat-6) 12%, var(--surface)), var(--surface) 70%)', border: '1px solid color-mix(in srgb, var(--cat-6) 24%, var(--border-2))', borderRadius: 'var(--radius-lg)', padding: '18px 20px' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap', marginBottom: 14 }}>
        ...
        <span style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-3)', background: 'var(--btn)', border: '1px solid var(--border-2)', padding: '2px 8px', borderRadius: 'var(--radius-xs)' }}>{a.ownIsHome ? tr.teams.home : tr.teams.away}</span>
            ...
            <div key={g.positionId} style={{ display: 'flex', alignItems: 'center', gap: 12, background: 'var(--btn)', border: '1px solid var(--border-2)', borderRadius: 'var(--radius-md)', padding: '12px 14px', flexWrap: 'wrap' }}>
              ...
              {ownPlayerId && (
                <button onClick={() => s.startBoardGame(a.leagueId, a.fixtureId, g.positionId, ownPlayerId, a.oppName)} title={tr.boardPanel.startTitle}
                  style={{ flexShrink: 0, display: 'flex', alignItems: 'center', gap: 7, background: 'var(--accent)', border: 'none', color: 'var(--accent-fg)', padding: '8px 14px', borderRadius: 'var(--radius-sm)', fontSize: 13, fontWeight: 800, cursor: 'pointer', fontFamily: 'inherit' }}>
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><polygon points="6 4 20 12 6 20 6 4" /></svg>
