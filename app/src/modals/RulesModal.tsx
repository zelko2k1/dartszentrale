    ...
    <div onClick={(e) => { if (e.target === e.currentTarget) s.closeRules(); }} style={{ position: 'fixed', inset: 0, background: 'var(--scrim)', backdropFilter: 'blur(6px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 62, padding: 24 }}>
      <div className="dh-pop" style={{ background: 'var(--surface)', border: '1px solid var(--border-2)', borderRadius: 'var(--radius-xl)', padding: 28, width: 480, maxWidth: '92vw', boxShadow: 'var(--shadow-card)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 16 }}>
          ...
          <div>
            <div style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-4)', fontWeight: 700, letterSpacing: '.08em', textTransform: 'uppercase' }}>{tr.trainingScr.rulesTitle}</div>
            <div style={{ fontSize: 'var(--fs-lg)', fontWeight: 800 }}>{mode.name}</div>
          </div>
        </div>
        <div style={{ background: 'var(--surface-2)', border: '1px solid var(--border-2)', borderRadius: 'var(--radius-md)', padding: '14px 16px', marginBottom: 16 }}>
          <div style={{ fontSize: 'var(--fs-2xs)', color: mode.color, fontWeight: 800, letterSpacing: '.06em', textTransform: 'uppercase', marginBottom: 5 }}>{tr.trainingScr.goal}</div>
          <div style={{ fontSize: 'var(--fs-base)', color: 'var(--text)', lineHeight: 1.5 }}>{rules.goal}</div>
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 11, marginBottom: 24 }}>
          {rules.lines.map((ln, i) => (
            <div key={i} style={{ display: 'flex', gap: 11, alignItems: 'flex-start' }}>
              ...
              <div style={{ fontSize: 'var(--fs-sm)', color: 'var(--text-2)', lineHeight: 1.5 }}>{ln}</div>
            </div>
          ))}
        </div>
        <div style={{ display: 'flex', gap: 12, justifyContent: 'flex-end' }}>
          <button className="dh-btn" onClick={() => s.closeRules()} style={{ background: 'var(--btn)', border: '1px solid var(--border-2)', color: 'var(--text)', padding: '12px 20px', borderRadius: 'var(--radius-md)', fontSize: 'var(--fs-base)', fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>{tr.trainingScr.close}</button>
        </div>
      </div>
    </div>
  );
