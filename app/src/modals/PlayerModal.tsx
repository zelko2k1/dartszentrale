      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 22 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8, minWidth: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <button className="dh-btn" onClick={() => s.cyclePlayerAvi(-1)} title={tr.modals.colorBack} aria-label={tr.modals.colorBack} style={{ minWidth: 44, minHeight: 44, borderRadius: 'var(--radius-md)', background: 'var(--btn)', border: '1px solid var(--border-2)', color: 'var(--text)', fontSize: 17, cursor: 'pointer', fontFamily: 'inherit' }}>‹</button>
            <button className="dh-btn" onClick={() => s.cyclePlayerAvi(1)} title={tr.modals.colorFwd} aria-label={tr.modals.colorFwd} style={{ minWidth: 44, minHeight: 44, borderRadius: 'var(--radius-md)', background: 'var(--btn)', border: '1px solid var(--border-2)', color: 'var(--text)', fontSize: 17, cursor: 'pointer', fontFamily: 'inherit' }}>›</button>
          ...
          {canPhoto ? (
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
              ...
              {photo && <button onClick={() => m.id && s.clearPhoto('player', m.id)} style={{ background: 'transparent', border: '1px solid var(--border-2)', color: 'var(--text-3)', padding: '7px 11px', borderRadius: 'var(--radius-sm)', fontSize: 12, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>{tr.modals.remove}</button>}
            </div>
          ) : isVerein && m.mode === 'add' ? (
      ...
      {confirmDel ? (
        <div style={{ background: 'color-mix(in srgb, var(--danger) 8%, transparent)', border: '1px solid color-mix(in srgb, var(--danger) 32%, transparent)', borderRadius: 'var(--radius-md)', padding: '14px 16px' }}>
          ...
          <div style={{ display: 'flex', gap: 12, justifyContent: 'flex-end' }}>
            <button className="dh-btn" onClick={() => setConfirmDel(false)} style={{ background: 'var(--btn)', border: '1px solid var(--border-2)', color: 'var(--text)', padding: '11px 18px', borderRadius: 'var(--radius-md)', fontSize: 14, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>{tr.common.cancel}</button>
            <button onClick={() => s.deletePlayer(m.id!)} style={{ background: 'var(--danger)', border: 'none', color: '#fff', padding: '11px 20px', borderRadius: 'var(--radius-md)', fontSize: 14, fontWeight: 800, cursor: 'pointer', fontFamily: 'inherit' }}>{tr.common.delete}</button>
          </div>
        </div>
      ) : (
        <ModalFooter
