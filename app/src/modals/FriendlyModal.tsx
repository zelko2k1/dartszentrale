  ...
  return (
    <Modal onClose={() => s.closeFriendly()} width={520} z={63} style={{ maxHeight: '88vh', overflow: 'auto' }}>
      ...
      {ownNames.length > 1 ? (
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginBottom: 16 }}>
          {ownNames.map((n) => {
            ...
            return <button key={n} onClick={() => s.setFriendlyField('ownTeam', n)} style={{ background: on ? 'color-mix(in srgb, var(--accent) 12%, transparent)' : 'var(--btn)', border: `1.5px solid ${on ? 'var(--accent)' : 'var(--border-2)'}`, color: on ? 'var(--success)' : 'var(--text-2)', padding: '8px 13px', borderRadius: 'var(--radius-pill)', fontSize: 'var(--fs-sub)', fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>{n}</button>;
          })}
        </div>
      ) : (
        <input className="dh-input" value={m.ownTeam} maxLength={60} onChange={(e) => s.setFriendlyField('ownTeam', e.target.value)} aria-label={tr.modals.ownTeam} placeholder={tr.modals.ownTeamPh} style={{ ...inputStyle, marginBottom: 16 }} />
      )}
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16, padding: '12px 16px', background: 'var(--btn)', borderRadius: 'var(--radius-md)', marginBottom: 16 }}>
        <div style={{ fontSize: 'var(--fs-body)', fontWeight: 600 }}>{m.homeIsOwn ? tr.modals.homeGame : tr.modals.awayGame}</div>
      ...
      <input className="dh-input" value={m.opponent} maxLength={60} onChange={(e) => s.setFriendlyField('opponent', e.target.value)} aria-label={tr.modals.opponent} placeholder={tr.modals.opponentPh} style={{ ...inputStyle, marginBottom: 8 }} />
      {matches.length > 0 ? (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 4, maxHeight: 180, overflowY: 'auto', marginBottom: 16 }}>
          {matches.map((n) => {
            ...
            return <button key={n} onClick={() => s.setFriendlyField('opponent', n)} style={{ textAlign: 'left', background: on ? 'color-mix(in srgb, var(--accent) 12%, transparent)' : 'var(--btn)', border: `1px solid ${on ? 'var(--accent)' : 'var(--border-2)'}`, borderRadius: 'var(--radius-md)', padding: '9px 12px', fontSize: 'var(--fs-sub)', fontWeight: 600, color: 'var(--text)', cursor: 'pointer', fontFamily: 'inherit' }}>{n}</button>;
          })}
        </div>
      ) : (
        <div style={{ fontSize: 'var(--fs-meta)', color: 'var(--text-5)', marginBottom: 16 }}>{tr.modals.noKnownOpp}</div>
      )}
      ...
      <input className="dh-input" value={m.loc} maxLength={80} onChange={(e) => s.setFriendlyField('loc', e.target.value)} aria-label={tr.modals.place} placeholder={tr.modals.placePh} style={{ ...inputStyle, marginBottom: 18 }} />

      <ModalFooter onCancel={() => s.closeFriendly()} onSave={() => s.saveFriendly()} saveDisabled={!canSave} saveLabel={tr.modals.create} />
      <div style={{ fontSize: 'var(--fs-badge)', color: 'var(--text-5)', marginTop: 10, lineHeight: 1.5 }}>
