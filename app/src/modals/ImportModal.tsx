...
export function ImportModal() {
  ...
  return (
    <Modal onClose={close} width={620} z={64} label={tr.modals.importTitle} style={{ maxHeight: '88vh', overflow: 'auto' }}>
      <ModalTitle>{tr.modals.importTitle}</ModalTitle>

      {result ? (
        // ── Ergebnis ──
        <>
          <div style={{ background: 'color-mix(in srgb, var(--success) 10%, transparent)', border: '1px solid color-mix(in srgb, var(--success) 40%, transparent)', borderRadius: 14, padding: 20, marginBottom: 18 }}>
        ...
        <>
          ...
          {error && (
            <div style={{ background: 'color-mix(in srgb, var(--danger) 10%, transparent)', border: '1px solid color-mix(in srgb, var(--danger) 40%, transparent)', color: 'var(--danger)', borderRadius: 11, padding: 14, fontSize: 13, marginBottom: 16 }}>{error}</div>
          )}

          {warning && (
            <div style={{ background: 'color-mix(in srgb, var(--warn) 12%, transparent)', border: '1px solid color-mix(in srgb, var(--warn) 45%, transparent)', color: 'var(--warn)', borderRadius: 11, padding: 14, fontSize: 13, marginBottom: 16, lineHeight: 1.5 }}>⚠️ {warning}</div>
          )}
            ...
            <>
              ...
              {(() => {
                ...
                if (si.willSwitchActive) {
                  ...
                  return (
                    <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10, background: 'color-mix(in srgb, var(--gold) 12%, transparent)', border: '1px solid color-mix(in srgb, var(--gold) 45%, transparent)', borderRadius: 12, padding: '12px 14px', marginBottom: 16, fontSize: 13, lineHeight: 1.5, color: 'var(--text-2)' }}>
                      ...
                      <span>
                        ...
                        {' '}<strong style={{ color: 'var(--warn)' }}>{tr.modals.attention}</strong>{tr.modals.activeSeason1}<strong style={{ color: 'var(--text)' }}>{si.archivedName}</strong>{tr.modals.archived1}<strong>{tr.modals.archivedWord}</strong>{tr.modals.archived2}
                      </span>
                    </div>
                  );
                }
              ...
              {conflicts.length > 0 && (
                <div style={{ background: 'color-mix(in srgb, var(--gold) 8%, transparent)', border: '1px solid color-mix(in srgb, var(--gold) 40%, transparent)', borderRadius: 12, padding: '12px 14px', marginBottom: 16 }}>
