    ...
    <Modal onClose={() => s.closeTeamModal()} width={520} z={61} style={{ maxHeight: '88vh', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      ...
      <div style={{ display: 'flex', gap: 12, marginBottom: 18 }}>
        <div style={{ flex: 1.4 }}>
          ...
          <input className="dh-input" value={m.name} onChange={(e) => s.setTeamField('name', e.target.value)} placeholder={tr.modals.teamNameEgPh} style={{ width: '100%', boxSizing: 'border-box', background: 'var(--btn)', border: '1px solid var(--border-2)', borderRadius: 11, padding: '12px 14px', color: 'var(--text)', fontSize: 14, fontFamily: 'inherit' }} />
        </div>
        <div style={{ flex: 1 }}>
          <FieldLabel note={tr.modals.optional}>{tr.modals.leagueField}</FieldLabel>
          <input className="dh-input" value={m.league} onChange={(e) => s.setTeamField('league', e.target.value)} placeholder={tr.modals.leagueNamePh} style={{ width: '100%', boxSizing: 'border-box', background: 'var(--btn)', border: '1px solid var(--border-2)', borderRadius: 11, padding: '12px 14px', color: 'var(--text)', fontSize: 14, fontFamily: 'inherit' }} />
        </div>
      </div>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 18 }}>
        {(Object.keys(TEAM_KINDS) as TeamKind[]).map((k) => {
          ...
          return (
            <button key={k} onClick={() => s.setTeamKind(k)} style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, background: on ? `color-mix(in srgb, ${def.color} 13%, transparent)` : 'var(--btn)', border: `1px solid ${on ? def.color : 'var(--border-2)'}`, color: on ? def.color : 'var(--text-3)', borderRadius: 11, padding: '10px 14px', fontSize: 13, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>
      ...
      {s.players.length === 0 && (
        <div style={{ background: 'var(--btn)', border: '1px dashed var(--border-strong)', borderRadius: 12, padding: 24, textAlign: 'center', color: 'var(--text-4)', fontSize: 13, marginBottom: 18 }}>{tr.modals.createPlayersFirst}</div>
      )}
