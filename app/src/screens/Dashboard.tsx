...
function VereinDashboard() {
  const s = useStore();
  const tr = useT();
  const MON3 = tr.format.mon3;
  const WD = tr.format.wdShort;
  const now = new Date();
  ...
  const events = dEvents
    ...
    .map(({ e, dt }) => {
      ...
      const rel = dayDiff === 0 ? tr.common.today : dayDiff === 1 ? tr.common.tomorrow : WD[dt.getDay()];
      return {
        ...
        meta: (e.time ? e.time + tr.common.oclock : '') + (e.time && e.loc ? ' · ' : '') + (e.loc || ''),
      };
    });
  ...
  const roleLabel = p.role && (ROLES as Record<string, { label: string }>)[p.role] ? (ROLES as Record<string, { label: string }>)[p.role].label : tr.dashboard.clubFallback;
  const firstName = u?.first?.trim() || u?.name?.trim() || s.settings.clubName || tr.dashboard.clubFallback;
  ...
  const adminLinks: { label: string; sub: string; icon: React.ReactNode; screen: Parameters<typeof s.go>[0] }[] = [
    { label: tr.dashboard.players, sub: tr.dashboard.inListCount(s.players.length), icon: <IconUserCheck size={20} />, screen: 'players' },
    { label: tr.dashboard.usersRights, sub: tr.dashboard.accountsCount(s.accounts.length), icon: <IconUsers size={20} />, screen: 'users' },
    { label: tr.dashboard.teams, sub: tr.dashboard.squadsCount(dTeams.length), icon: <IconShield size={20} />, screen: 'teams' },
    { label: tr.dashboard.leaguesCups, sub: tr.dashboard.competitionsCount(dLeagues.length), icon: <IconTrophy size={20} />, screen: 'leagues' },
    { label: tr.nav.settings, sub: tr.dashboard.settingsSub, icon: <IconSettings size={20} />, screen: 'settings' },
  ];
  ...
  const MyStatsCard = myAgg && myPlayer ? (
    <div style={cardStyle}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 10, marginBottom: 14 }}>
        {[
          { v: String(myAgg.games), l: tr.dashboard.games },
          { v: `${myAgg.wins}–${myAgg.losses}`, l: tr.dashboard.winLoss },
          { v: String(myAgg.c180), l: tr.dashboard.count180 },
          { v: String(myAgg.high || '–'), l: tr.dashboard.highFinish },
...
function LocalDashboard() {
  ...
  const quickItems: { key: string; color: string; iconPath: string | null; title: string; sub: string; onClick: () => void }[] = [
    { key: 'x01-3', color: accent, iconPath: null, title: '501 · Double Out · Best of 3', sub: tr.dashboard.x01Match, onClick: () => startX01(3) },
    { key: 'x01-5', color: accent, iconPath: null, title: '501 · Double Out · Best of 5', sub: tr.dashboard.x01Match, onClick: () => startX01(5) },
