    ...
    <div style={{ padding: isPhone ? '18px 14px' : '28px 32px', maxWidth: 980, margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 22 }}>
        ...
        <div>
          ...
          <h1 style={{ margin: '2px 0 0', fontSize: 'var(--fs-page)', fontWeight: 800, letterSpacing: '-.02em' }}>{t.setupTitle}</h1>
        </div>
      </div>

      {!enoughPlayers && (
        <div style={{ background: 'color-mix(in srgb, var(--danger) 12%, transparent)', border: '1px solid color-mix(in srgb, var(--danger) 40%, transparent)', color: 'var(--text)', borderRadius: 'var(--radius-md)', padding: '12px 16px', marginBottom: 16, fontSize: 'var(--fs-body)' }}>{t.tooFewPlayers}</div>
      )}
      ...
      <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius-lg)', padding: '8px 22px', marginBottom: 18 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16, padding: '16px 0', flexWrap: 'wrap' }}>
          <div style={{ fontSize: 'var(--fs-body)', fontWeight: 600 }}>{t.nameLabel}</div>
          <input aria-label={t.nameLabel} value={su.name} onChange={(e) => s.setTournamentSetup({ name: e.target.value })} placeholder={t.namePlaceholder}
            style={{ flex: '1 1 240px', maxWidth: 320, background: 'var(--btn)', color: 'var(--text)', border: '1px solid var(--border-2)', borderRadius: 'var(--radius-md)', padding: '10px 13px', fontSize: 'var(--fs-body)', fontFamily: 'inherit', outline: 'none' }} />
        </div>
        ...
        <Row label={t.boardsLabel}>
          <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap', justifyContent: 'flex-end' }}>
            <span style={{ fontSize: 'var(--fs-meta)', color: 'var(--text-4)', marginRight: 4 }}>{t.boardsHint(su.boardCount)}</span>
      ...
      <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius-lg)', padding: '8px 22px', marginBottom: 22 }}>
        <SectionHeading style={{ padding: '16px 0 12px' }}>{t.playersLabel}</SectionHeading>
            ...
            return (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 11, padding: '10px 12px', background: 'var(--btn)', border: '1px solid var(--border-2)', borderRadius: 'var(--radius-md)' }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <select aria-label={tr.trainingScr.playerN(i + 1)} value={su.picks[i]} onChange={(e) => s.setTournamentPick(i, Number(e.target.value))} style={{ width: '100%', background: 'transparent', color: 'var(--text)', border: 'none', fontSize: 'var(--fs-body)', fontWeight: 700, fontFamily: 'inherit', cursor: 'pointer', outline: 'none', padding: 0 }}>
