    ...
    <div style={{ padding: '28px 32px', maxWidth: 1080, margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 20, marginBottom: 24 }}>
        ...
        <div style={{ textAlign: 'center', background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 14, padding: '14px 22px' }}>
          <div style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: 32, fontWeight: 800, color: '#2BD377' }}>{agg.avg ? agg.avg.toFixed(1) : '–'}</div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: 12, marginBottom: 20 }}>
        {stats.map((st2) => (
          <div key={st2.label} style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 14, padding: 15 }}>
            <div style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: 22, fontWeight: 800, color: st2.color }}>{st2.value}</div>
          ...
          {scoring.map((st2) => (
            <div key={st2.label} style={{ background: 'var(--btn)', border: '1px solid var(--border-2)', borderRadius: 12, padding: '14px 16px', display: 'flex', alignItems: 'center', gap: 13 }}>
              ...
              <div style={{ minWidth: 0 }}>
                <div style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: 24, fontWeight: 800, color: 'var(--text)', lineHeight: 1 }}>{st2.value}</div>
              ...
              {agg.recent.slice().reverse().map((f, i, arr) => {
                ...
                return (
                  <div key={i} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 7, height: '100%', justifyContent: 'flex-end' }}>
                    <span style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: 10, color: 'var(--text-4)', fontWeight: 700 }}>{f.avg ? f.avg.toFixed(0) : '0'}</span>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
              {agg.recent.map((g, i) => (
                <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 4px', borderBottom: '1px solid var(--hairline)' }}>
                  ...
                  <span style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: 13, color: 'var(--text-3)' }}>Ø {g.avg ? g.avg.toFixed(1) : '–'}</span>
                </div>
              ))}
            </div>
          )}
