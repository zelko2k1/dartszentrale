...
function tagText(m: TrainMode) {
  if (m.solo && m.versus) return 'Solo · Mehrspieler';
  if (m.versus) return 'Mehrspieler';
  return 'Solo';
}
...
export function Training() {
  ...
  return (
    <div style={{ padding: '28px 32px', maxWidth: 1100, margin: '0 auto' }}>
      <div style={{ marginBottom: 6, fontSize: 12, color: 'var(--text-4)', fontWeight: 700, letterSpacing: '.1em', textTransform: 'uppercase' }}>Übungen</div>
      <h1 style={{ margin: '0 0 22px', fontSize: 27, fontWeight: 800, letterSpacing: '-.02em' }}>Trainingsspiele</h1>
      <div style={{ fontSize: 12, color: 'var(--text-3)', fontWeight: 800, letterSpacing: '.08em', textTransform: 'uppercase', margin: '4px 0 14px' }}>Solo-Training</div>
      <Grid modes={training} />
      <div style={{ fontSize: 12, color: 'var(--text-3)', fontWeight: 800, letterSpacing: '.08em', textTransform: 'uppercase', margin: '4px 0 14px' }}>Mehrspieler · Solo &amp; bis 8 Spieler</div>
      <Grid modes={party} />
    </div>
  );
}
    ...
    <div onClick={(e) => { if (e.target === e.currentTarget) s.closeRules(); }} style={{ position: 'fixed', inset: 0, background: 'rgba(8,10,12,.78)', backdropFilter: 'blur(6px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 62, padding: 24 }}>
      <div className="dh-pop" style={{ background: 'var(--surface)', border: '1px solid var(--border-2)', borderRadius: 20, padding: 28, width: 480, maxWidth: '92vw', boxShadow: '0 30px 70px rgba(0,0,0,.55)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 16 }}>
          ...
          <div>
            <div style={{ fontSize: 12, color: 'var(--text-4)', fontWeight: 700, letterSpacing: '.08em', textTransform: 'uppercase' }}>Spielregeln</div>
        ...
        <div style={{ background: 'var(--surface-2)', border: '1px solid var(--border-2)', borderRadius: 12, padding: '14px 16px', marginBottom: 16 }}>
          <div style={{ fontSize: 11, color: mode.color, fontWeight: 800, letterSpacing: '.06em', textTransform: 'uppercase', marginBottom: 5 }}>Ziel</div>
        ...
        <div style={{ display: 'flex', gap: 12, justifyContent: 'flex-end' }}>
          <button className="dh-btn" onClick={() => s.closeRules()} style={{ background: 'var(--btn)', border: '1px solid var(--border-2)', color: 'var(--text)', padding: '12px 20px', borderRadius: 11, fontSize: 14, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>Schließen</button>
        </div>
      </div>
    </div>
  );
