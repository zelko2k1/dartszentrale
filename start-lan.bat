...
if errorlevel 1 (
  echo [DartsZentrale] Node.js wurde nicht gefunden.
...
if not exist ".env.local" (
  ...
  echo [DartsZentrale] app\.env.local angelegt ^(VITE_PB_URL=http://127.0.0.1:8090^)
) else (
  findstr /b /c:"VITE_PB_URL=" ".env.local" >nul 2>nul || >>".env.local" echo VITE_PB_URL=http://127.0.0.1:8090
)

if not exist "node_modules" (
  echo [DartsZentrale] Installiere Abhaengigkeiten ^(einmalig, kann etwas dauern^)...
...
if exist "%~dp0pocketbase\pocketbase.exe" (
  echo [DartsZentrale] Starte PocketBase auf http://127.0.0.1:8090 ...
  start "DartsZentrale PocketBase" /D "%~dp0pocketbase" cmd /k "pocketbase.exe serve --automigrate=0 --http=127.0.0.1:8090 --dir=pb_data --migrationsDir=pb_migrations --hooksDir=pb_hooks"
) else (
  echo [DartsZentrale] Hinweis: pocketbase.exe nicht gefunden - fuer den Vereinsmodus wird es gebraucht.
  echo            ^(Das Frontend startet trotzdem; im Lokalmodus ist kein Server noetig.^)
)

REM --- Frontend ---
echo [DartsZentrale] Starte Frontend auf http://localhost:4173 ...
start "DartsZentrale Frontend" /D "%~dp0app" cmd /k "set HOST=127.0.0.1&& set PORT=4173&& node serve-dist.mjs"
...
echo.
echo [DartsZentrale] Laeuft. Die Fenster "DartsZentrale PocketBase" und "DartsZentrale Frontend" offen lassen,
...
:error
echo.
echo [DartsZentrale] FEHLER beim Einrichten. Siehe Meldungen oben.
pause
exit /b 1
