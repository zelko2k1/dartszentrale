# ============================================================================
# DartsZentrale - Gefuehrte Vereinsmodus-Einrichtung im LAN (Windows / PowerShell)
# [ PRODUKTIV / OPS ]
# ----------------------------------------------------------------------------
# Richtet DartsZentrale auf DIESEM PC komplett ein - mit Abfragen, bis alles laeuft
...
$suEmailDef = if ($env:PB_SU_EMAIL) { $env:PB_SU_EMAIL } else { "admin@dartszentrale.local" }
...
@echo off
chcp 65001 >nul
title DartsZentrale Server
start "DartsZentrale PocketBase" /D "%~dp0pocketbase" cmd /k "pocketbase.exe serve --automigrate=0 --http=$bind`:$PbPort --dir=pb_data --migrationsDir=pb_migrations --hooksDir=pb_hooks"
start "DartsZentrale Frontend" /D "%~dp0app" cmd /k "set HOST=$bind&& set PORT=$WebPort&& node serve-dist.mjs"
...
if (-not $ok) { Write-Error "PocketBase nicht erreichbar - laeuft das Fenster 'DartsZentrale PocketBase'?"; exit 1 }
...
$lnk = [System.IO.Path]::Combine($env:APPDATA,'Microsoft\Windows\Start Menu\Programs\Startup','DartsZentrale.lnk')
...
Write-Host ""
Write-Host "OK - DartsZentrale-Vereinsmodus laeuft:"
...
Write-Host "Hinweis Update: 'dartszentrale-update-*.tar.gz' nach '$Root\updates\' legen -> Einstellungen -> 'App & Updates' -> Installieren."
